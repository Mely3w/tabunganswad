...
function TabungModal({
  ...
  return (
    ...
    >
      <div
        className="w-full bg-card rounded-t-3xl px-6 pt-6 pb-10 max-h-[90%] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-foreground">{stepTitle[step]}</h3>
          </div>
          {step !== "bukti" && (
        ...
        {step !== "bukti" && (
          <div className="flex items-center gap-2 mb-5">
            {(["input", "konfirmasi"] as TabungStep[]).map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div
                  className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
                  style={{
                    backgroundColor: step === s || (step === "konfirmasi" && i === 0) ? "#1A3A6B" : "#DDE6F0",
                    color: step === s || (step === "konfirmasi" && i === 0) ? "#fff" : "#6B7E95",
                  }}
                >
                  {step === "konfirmasi" && i === 0 ? <Check size={12} /> : i + 1}
                </div>
                <span className="text-xs font-medium" style={{ color: step === s ? "#1A3A6B" : "#6B7E95" }}>
                  {s === "input" ? "Nominal" : "Konfirmasi"}
                </span>
        ...
        {step === "input" && (
          <>
            ...
            <div className="rounded-2xl overflow-hidden mb-4" style={{ border: "1.5px solid #1A3A6B22" }}>
              <div className="px-4 py-2.5 flex items-center gap-2" style={{ background: "linear-gradient(135deg, #1A3A6B, #2563EB)" }}>
                <CreditCard size={13} className="text-blue-200" />
                <span className="text-white text-xs font-semibold">Rekening Tabungan Swad</span>
              </div>
              <div className="px-4 py-3 flex items-center justify-between">
                <div>
                  <p className="text-xs text-muted-foreground">Abigail Nirwana</p>
                  <p className="text-base font-bold font-mono tracking-widest" style={{ color: "#1A3A6B" }}>1023 - 9182</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">Kelas XI Akuntansi</p>
                  <p className="text-xs font-medium text-foreground">SMK Swadaya Semarang</p>
                </div>
              </div>
            </div>
            ...
            {/* Input nominal */}
            <div className="mb-4">
              <label className="text-xs font-semibold text-muted-foreground block mb-2">Jumlah Tabungan</label>
            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {[10000, 20000, 50000, 100000, 200000, 500000].map((v) => (
                <button
                  ...
                  className="text-xs font-semibold rounded-xl py-2.5 border transition-all"
                  style={{
                    borderColor: amount === String(v) ? "#1A3A6B" : "var(--border)",
                    backgroundColor: amount === String(v) ? "#D6E4F7" : "transparent",
                    color: amount === String(v) ? "#1A3A6B" : "var(--foreground)",
                  }}
                >
                  {formatRupiah(v)}
                </button>
            ...
            {/* Jam operasional */}
            <div className="flex items-center gap-3 bg-muted rounded-2xl px-4 py-3 mb-5">
              <Clock size={15} className="text-muted-foreground flex-shrink-0" />
              <p className="text-xs text-muted-foreground">Bank sekolah buka <strong className="text-foreground">Senin–Jumat, 08.00–12.00</strong></p>
            </div>
        ...
        {step === "konfirmasi" && (
          <>
            <div className="bg-blue-50 rounded-2xl px-4 py-3 mb-5 flex gap-3">
              <div className="w-5 h-5 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-white text-[10px] font-bold">i</span>
              </div>
              <p className="text-xs text-blue-700 leading-relaxed">
                Setelah konfirmasi, tunjukkan layar ini ke <strong>petugas Tabungan Swad</strong> saat menyetorkan uangmu.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-border overflow-hidden mb-5">
              <div className="px-5 py-3 border-b border-border bg-muted">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Rincian Setoran</p>
              </div>
              <div className="px-5 py-4 space-y-3">
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Nama</span>
                  <span className="text-sm font-semibold text-foreground">Abigail Nirwana</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">No. Rekening</span>
                  <span className="text-sm font-mono font-bold text-foreground">1023 - 9182</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Kelas</span>
                  <span className="text-sm font-semibold text-foreground">XI Akuntansi / SMK Swadaya Semarang</span>
                </div>
                <div className="flex justify-between pt-2 border-t border-border">
                  <span className="text-sm font-bold text-foreground">Jumlah Tabungan</span>
                  <span className="text-base font-bold" style={{ color: "#1A3A6B" }}>{formatRupiah(Number(amount))}</span>
                </div>
              </div>
            </div>
        ...
        {/* STEP 3 — Bukti Transaksi Digital */}
        {step === "bukti" && (
          <>
            <div className="flex flex-col items-center mb-5">
              ...
              <p className="text-base font-bold text-foreground">Tabungan Dikonfirmasi!</p>
              <p className="text-xs text-muted-foreground mt-1">Serahkan uang tunai ke petugas Tabungan Swad</p>
            </div>

            {/* Bukti — desain struk */}
            <div className="rounded-2xl overflow-hidden mb-5" style={{ border: "1.5px solid #1A3A6B22" }}>
              {/* Header struk */}
              <div className="px-5 py-4 text-center" style={{ background: "linear-gradient(135deg, #1A3A6B, #2563EB)" }}>
                <p className="text-white text-xs font-semibold tracking-wide uppercase mb-0.5">TABUNGAN SWAD</p>
                <p className="text-blue-200 text-[10px]">SMK Swadaya Semarang</p>
              </div>
                ...
                <div className="absolute inset-x-0 top-0 flex">
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="flex-1 h-3" style={{
                      background: i % 2 === 0
                        ? "radial-gradient(circle at 50% 0%, #1A3A6B 50%, transparent 50%)"
                        : "transparent"
                    }} />
                  ))}
                </div>
              ...
              <div className="px-5 py-4 space-y-2.5 bg-card">
                <div className="flex justify-between">
                  <span className="text-xs text-muted-foreground">No. Referensi</span>
                  <span className="text-xs font-mono font-bold text-foreground">{noRef}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-muted-foreground">Tanggal</span>
                  <span className="text-xs font-medium text-foreground">{tgl}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-muted-foreground">Waktu</span>
                  <span className="text-xs font-medium text-foreground">{jam} WIB</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-muted-foreground">Nama</span>
                  <span className="text-xs font-semibold text-foreground">Abigail Nirwana</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-xs font-medium text-foreground">XI Akuntansi</span>
                </div>
                <div className="border-t border-dashed border-border pt-2.5 mt-1 flex justify-between">
                  <span className="text-sm font-bold text-foreground">Jumlah Setoran</span>
                  <span className="text-sm font-bold" style={{ color: "#16A34A" }}>{formatRupiah(Number(amount))}</span>
                </div>
              </div>
              ...
              <div className="relative h-3 bg-card">
                <div className="absolute inset-x-0 bottom-0 flex">
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="flex-1 h-3" style={{
                      background: i % 2 === 0
                        ? "radial-gradient(circle at 50% 100%, #1A3A6B 50%, transparent 50%)"
                        : "transparent"
                    }} />
                  ))}
                </div>
              ...
              <div className="px-5 py-3 text-center bg-muted">
                <p className="text-[10px] text-muted-foreground">Simpan bukti ini sebagai tanda terima setoran</p>
              </div>
            </div>
...
function ReminderModal({ config, onSave, onClose }: {
  config: ReminderConfig;
  onSave: (c: ReminderConfig) => void;
  onClose: () => void;
}) {
  const [local, setLocal] = useState<ReminderConfig>({ ...config });

  const toggleHari = (h: number) => {
    setLocal((prev) => ({
      ...prev,
      hari: prev.hari.includes(h) ? prev.hari.filter((x) => x !== h) : [...prev.hari, h].sort(),
    }));
  };

  const frekuensiOptions: { id: Frekuensi; label: string; desc: string }[] = [
    { id: "harian", label: "Setiap Hari", desc: "Pengingat dikirim setiap hari" },
    { id: "mingguan", label: "Setiap Minggu", desc: "Pilih hari yang kamu inginkan" },
    { id: "bulanan", label: "Setiap Bulan", desc: "Sekali di awal bulan" },
  ];

  return (
    <div className="absolute inset-0 z-50 flex items-end" style={{ background: "rgba(0,0,0,0.5)" }}>
      <div className="w-full bg-card rounded-t-3xl px-6 pt-6 pb-10 max-h-[88%] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold text-foreground">Atur Pengingat Menabung</h3>
          <button onClick={onClose} className="p-1"><X size={20} className="text-muted-foreground" /></button>
        </div>

        {/* Toggle aktif */}
        <div className="flex items-center justify-between bg-muted rounded-2xl px-4 py-3.5 mb-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-amber-100 flex items-center justify-center">
              <AlarmClock size={18} color="#D97706" />
            </div>
            <div>
              <p className="text-sm font-bold text-foreground">Pengingat Menabung</p>
              <p className="text-xs text-muted-foreground">{local.aktif ? "Aktif" : "Nonaktif"}</p>
            </div>
          </div>
          <button onClick={() => setLocal((p) => ({ ...p, aktif: !p.aktif }))}>
            {local.aktif
        ...
        {local.aktif && (
          <>
            {/* Frekuensi */}
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Frekuensi</p>
            <div className="space-y-2 mb-5">
              {frekuensiOptions.map((f) => (
                <button
                  key={f.id}
                  onClick={() => setLocal((p) => ({ ...p, frekuensi: f.id }))}
                  className="w-full flex items-center gap-3 rounded-xl px-4 py-3 border transition-colors text-left"
                  style={{
                    borderColor: local.frekuensi === f.id ? "#1A3A6B" : "var(--border)",
                    backgroundColor: local.frekuensi === f.id ? "#D6E4F7" : "transparent",
                  }}
                >
                  <div
                    className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0"
                    style={{ borderColor: local.frekuensi === f.id ? "#1A3A6B" : "#D1D5DB" }}
                  >
                    {local.frekuensi === f.id && (
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: "#1A3A6B" }} />
                    )}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold" style={{ color: local.frekuensi === f.id ? "#1A3A6B" : "var(--foreground)" }}>
                      {f.label}
                    </p>
            ...
            {/* Pilih hari (mingguan) */}
            {local.frekuensi === "mingguan" && (
              <div className="mb-5">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Pilih Hari</p>
                <div className="flex gap-2">
                  {hariList.map((h, i) => (
                    <button
                      key={h}
                      onClick={() => toggleHari(i)}
                      className="flex-1 py-2 rounded-xl text-xs font-bold transition-all"
                      style={{
                        backgroundColor: local.hari.includes(i) ? "#1A3A6B" : "#DDE6F0",
                        color: local.hari.includes(i) ? "#fff" : "#6B7E95",
                      }}
                    >
                      {h}
                    </button>
            ...
            {/* Pilih tanggal (bulanan) */}
            {local.frekuensi === "bulanan" && (
              <div className="mb-5">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Tanggal Setiap Bulan</p>
                <div className="grid grid-cols-7 gap-1.5">
                  {Array.from({ length: 28 }, (_, i) => i + 1).map((t) => (
                    <button
                      key={t}
                      onClick={() => setLocal((p) => ({ ...p, tanggal: t }))}
                      className="aspect-square rounded-lg text-xs font-bold transition-all"
                      style={{
                        backgroundColor: local.tanggal === t ? "#1A3A6B" : "#DDE6F0",
                        color: local.tanggal === t ? "#fff" : "#6B7E95",
                      }}
                    >
                      {t}
                    </button>
            ...
            {/* Waktu */}
            <div className="mb-6">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Waktu Pengingat</p>
              <div className="flex gap-2 flex-wrap">
                {["06:00", "07:00", "08:00", "12:00", "15:00", "18:00", "20:00"].map((t) => (
                  <button
                    key={t}
                    onClick={() => setLocal((p) => ({ ...p, waktu: t }))}
                    className="px-3 py-2 rounded-xl text-xs font-semibold border transition-all"
                    style={{
                      borderColor: local.waktu === t ? "#1A3A6B" : "var(--border)",
                      backgroundColor: local.waktu === t ? "#D6E4F7" : "transparent",
                      color: local.waktu === t ? "#1A3A6B" : "var(--foreground)",
                    }}
                  >
                    {t}
                  </button>
...
function TransferScreen({
  ...
  const handleSelect = (c: typeof contacts[0]) => {
    setSelected(c);
    setStep("amount");
  };
        ...
        <div className="px-4">
          <div className="bg-card rounded-2xl shadow-sm overflow-hidden mb-4">
            <div className="px-4 py-3 border-b border-border">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Kontak Favorit</p>
            </div>
            {contacts.map((c) => (
              <button
                key={c.id}
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <div className="text-xs text-muted-foreground font-mono">{c.account}</div>
                </div>
                <ChevronRight size={16} className="text-muted-foreground" />
              </button>
            ))}
          ...
          <div className="bg-card rounded-2xl shadow-sm px-4 py-4">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Nomor Rekening</p>
            <div className="flex gap-2">
              <input
      ...
      {step === "amount" && selected && (
        <div className="px-4">
          <div className="bg-card rounded-2xl shadow-sm px-4 py-4 mb-4 flex items-center gap-3">
            <Avatar initial={selected.initial} color={selected.color} size={44} />
            <div>
          ...
          <div className="bg-card rounded-2xl shadow-sm px-4 py-5 mb-4">
            <label className="text-xs font-medium text-muted-foreground block mb-2">Jumlah Transfer</label>
          ...
          <div className="bg-card rounded-2xl shadow-sm px-4 py-4 mb-5">
            <label className="text-xs font-medium text-muted-foreground block mb-2">Catatan (opsional)</label>
            <input
              className="w-full bg-muted rounded-xl px-4 py-3 text-sm text-foreground outline-none"
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-24" style={{ scrollbarWidth: "none" }}>
      <div className="px-4 pt-7 pb-5">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Semua transaksi kamu</p>
      </div>
      ...
      <div className="mx-4 grid grid-cols-2 gap-3 mb-4">
        <div className="bg-card rounded-2xl shadow-sm px-4 py-3">
          ...
          <div className="text-sm font-bold text-green-600">+{formatRupiah(totalMasuk)}</div>
        </div>
        <div className="bg-card rounded-2xl shadow-sm px-4 py-3">
          <div className="flex items-center gap-2 mb-1">
          ...
          <div className="text-sm font-bold text-red-500">-{formatRupiah(totalKeluar)}</div>
        </div>
      </div>
        ...
        <div className="flex gap-2 bg-muted rounded-2xl p-1">
          {(["semua", "masuk", "keluar"] as const).map((f) => (
            <button
              ...
              style={{
                backgroundColor: filter === f ? "#1A3A6B" : "transparent",
      ...
      {/* Transaction list */}
      <div className="mx-4 bg-card rounded-2xl shadow-sm px-4 divide-y divide-border">
        {filtered.map((tx) => (
          <TransactionItem key={tx.id} tx={tx} />
        ))}
        {filtered.length === 0 && (
          <div className="py-10 text-center text-sm text-muted-foreground">Tidak ada transaksi</div>
        )}
      </div>
    </div>
  );
...
export default function App() {
 ...
 const [liveTransactions, setLiveTransactions] =
  ...
  return (
    <div className="min-h-[100dvh] bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center p-0 sm:p-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      {/* Phone frame */}
      <div
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  size={22}
                  color={active ? "#1A3A6B" : "#9CA3AF"}
                  strokeWidth={active ? 2.5 : 1.75}
                />
                <span
                  className="text-[10px] font-semibold"
                  style={{ color: active ? "#1A3A6B" : "#9CA3AF" }}
                >
                  {item.label}
                </span>
                {active && (
                  <div className="absolute bottom-2 w-1 h-1 rounded-full" style={{ backgroundColor: "#1A3A6B" }} />
                )}
              </button>
            );
          })}
