...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  if (import.meta.env.PROD && !apiBaseUrl) {
    throw new Error("Alamat server belum diatur. Tambahkan VITE_API_URL di pengaturan Netlify.");
  }
  ...
  try {
    ...
    throw new Error("Tidak dapat terhubung ke server. Periksa alamat server dan koneksi internet.");
  }
